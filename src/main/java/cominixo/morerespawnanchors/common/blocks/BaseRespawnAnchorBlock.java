...
public class BaseRespawnAnchorBlock extends Block {
	...
	private static final ImmutableList<Vector3i> field_242676_b = ImmutableList.of(new Vector3i(0, 0, -1),
			new Vector3i(-1, 0, 0), new Vector3i(0, 0, 1), new Vector3i(1, 0, 0), new Vector3i(-1, 0, -1),
			new Vector3i(1, 0, -1), new Vector3i(-1, 0, 1), new Vector3i(1, 0, 1));
	private static final ImmutableList<Vector3i> field_242677_c = (new Builder<Vector3i>()).addAll(field_242676_b)
			.addAll(field_242676_b.stream().map(Vector3i::down).iterator())
			.addAll(field_242676_b.stream().map(Vector3i::up).iterator()).add(new Vector3i(0, 1, 0)).build();
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (handIn == Hand.MAIN_HAND && !isValidFuel(state.getBlock(), itemstack) && isValidFuel(state.getBlock(), player.getHeldItem(Hand.OFF_HAND))) {
			return ActionResultType.PASS;
		} else if (isValidFuel(state.getBlock(), itemstack) && notFullyCharged(state)) {
	...
	protected static boolean isValidFuel(Block blockIn, ItemStack stack) {
		return blockIn == MRABlocks.NETHERITE_RESPAWN_ANCHOR.get() ? stack.getItem() == Items.GLOWSTONE : stack.getItem() == Items.ENDER_PEARL;
	}

	private static boolean notFullyCharged(BlockState state) {
		return state.getBlock() == MRABlocks.END_RESPAWN_ANCHOR.get() ? state.get(CHARGES) < 4 : state.get(CHARGES) < 12;
	}
	...
	private static Optional<Vector3d> func_242678_a(EntityType<?> type, ICollisionReader collisionReader, BlockPos pos,
